...
const DEFAULT_COLUMNS: ColumnConfig[] = [
  ...
  { id: 'candidate_id', label: 'Mã UV', width: 120, visible: false }, // Đã có cột này nên ẩn hiển thị ở tên
           ...
           {detailLoading ? (
             ...
             <>
               ...
               <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-24 scrollbar-thin">
                  ...
                  {/* 1. QUY TRÌNH TRẠNG THÁI (FUNNEL) - 1 HÀNG NGANG */}
                  <section className="bg-white p-0 rounded-2xl">
                    ...
                    <div className="flex items-stretch gap-2 overflow-x-auto pb-2 scrollbar-thin">
                      {funnelSteps.map(step => {
                        // Xác định xem đây có phải trạng thái tiêu cực không
                        const isNegative = step.key === 'reject_offer' || step.key === 'unqualified';
                        // Logic class active
                        const activeClass = isNegative
                            ? 'border-gray-500 bg-gray-500 text-white shadow-md font-bold ring-2 ring-gray-200' // Màu xám cho tiêu cực
                            : 'border-blue-600 bg-blue-600 text-white shadow-md font-bold ring-2 ring-blue-100'; // Màu xanh cho tích cực
                  ...
                  {/* 2. NGÀY QUAN TRỌNG & LÝ DO (ĐƯỢC KÉO LÊN) */}
                  <section>
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            ...
                            {/* Lý do (Từ chối / Không đạt) */}
                            <div className="space-y-3">
                                <div>
                                    <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Lý do từ chối Offer</label>
                                    <input className="w-full p-2.5 border rounded-xl mt-1 outline-none bg-gray-50 focus:bg-white focus:border-gray-500 transition placeholder:text-gray-300 text-sm" placeholder="Nhập lý do ứng viên từ chối..." value={formData.reason_rejected_offer || ''} onChange={e => handleChange('reason_rejected_offer', e.target.value)} />
                                </div>
                                <div>
                                    <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Lý do không đạt (Loại)</label>
                                    <input className="w-full p-2.5 border rounded-xl mt-1 outline-none bg-gray-50 focus:bg-white focus:border-gray-500 transition placeholder:text-gray-300 text-sm" placeholder="Nhập lý do loại ứng viên..." value={formData.reason_unqualified || ''} onChange={e => handleChange('reason_unqualified', e.target.value)} />
                                </div>
                            </div>
                        </div>
                  </section>
                  ...
                  {/* 5. THÔNG TIN HỆ THỐNG (XUỐNG CUỐI CÙNG) */}
...
function StatusBadge({ cand }: { cand: any }) {
    ...
    if (cand.unqualified) return <span className={`${common} bg-gray-500 text-white`}>KHÔNG ĐẠT</span>; // Màu xám
    if (cand.reject_offer) return <span className={`${common} bg-gray-500 text-white`}>TỪ CHỐI</span>; // Màu xám
